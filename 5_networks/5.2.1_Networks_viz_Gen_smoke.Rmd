...
```{r, echo=FALSE}
# load("/Users/alicesommer/Desktop/DACOMP_cluster/results/ratio_matrix_test_save3.RData")
# # check sequence names differentially abundant
# diff_node_names <- rownames(tax_table(ps_Genus_prune)[!is.na(ratio_matrix_test_save[1,]),])
# length(diff_node_names)
```

```{r, echo=FALSE}
# row_sum_1 <- apply(props_W$input$normCounts1, 1,sum)
# row_sum_2 <- apply(props_W$input$normCounts2, 1,sum)
# norm_counts_1 <- clr(props_W$input$normCounts1 + .5)
# norm_counts_2 <- clr(props_W$input$normCounts2 + .5)
# 
# norm_counts_1[props_W$input$normCounts1 == 0] <- 0
# norm_counts_2[props_W$input$normCounts2 == 0] <- 0
# 
# props_W$input$normCounts1 <- norm_counts_1
# props_W$input$normCounts2 <- norm_counts_2
```

```{r, echo=FALSE}
# which(tax_table(ps_Genus_prune)[,"Class"] == "Mollicutes")
# tax_table(ps_Genus_prune)[which(tax_table(ps_Genus_prune)[,"Class"] == "Mollicutes"),]
```
...
```
```{r}
load("/Users/alicesommer/Desktop/DACOMP_cluster/results/PM_connect_seq.RData")
sum(names_clust_PM %in% names_clust)
overlap_seq <- names_clust[which(names_clust_PM %in% names_clust)]
overlap_seq

props_W$clustering$clust1[overlap_seq]
props_W$clustering$clust2[overlap_seq]
```
...
corrplot(cor_1, method = "color", tl.col = "grey", mar=c(0,0,5,0),
         col = col1(100), title = "Non-smoking (community 1)")
```
