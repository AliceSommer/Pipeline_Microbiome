...
overall_plot <- plot(props_W, 
     rmSingles = "inboth",
     layout = "spring", repulsion = .85, 
...
``` r
props_W_igraph1 <- graph_from_adjacency_matrix(props_W$input$adjaMat1, weighted = TRUE)
props_W_igraph2 <- graph_from_adjacency_matrix(props_W$input$adjaMat1, weighted = TRUE)

largest_comp1 <- names(which(clusters(props_W_igraph1)$membership == 1))
largest_comp2 <- names(which(clusters(props_W_igraph2)$membership == 1))

labels_1 <- as.factor(tax_table(ps_Genus_prune)[, 'Genus'])
names(labels_1) <- rownames(tax_table(ps_Genus_prune)[, 'Genus'])
# labels_1[-c(116,5,47)] <- NA

plot(props_W,
     # rmSingles = "inboth",
     layout = "spring", repulsion = .85, 
     # sameLayout = TRUE, layoutGroup = 1, 
     labels = names(labels_1[largest_comp1]), labelScale = FALSE , cexLabels = 1,
     featVecCol = phyl_ps_prune, nodeColor = "feature", 
     nodeTransp = 40,
     borderCol = "gray40", highlightHubs = FALSE, 
     nodeSize = "clr",
     cexNodes = .5, cexTitle = 2,
     edgeTranspLow = 30, edgeTranspHigh = 30,
     nodeShape = c("circle", "triangle"),
     featVecShape = shapeVec,
     mar = c(6,5,3,2), groupNames = c("Polluted air", "Clean air"),
     nodeFilter = "names", nodeFilterPar = largest_comp1)
```
...
``` r
order_labels <- plot_zoom$labels$labels1 ## save the ordered lanels and then replug in figure above 

plot(props_W, 
     sameLayout = TRUE, layoutGroup = 2, 
     repulsion = .8,
     # charToRm = "Seq",
     labels = as.character(labels_1[order_labels]),
     labelFont = 1, 
     cexLabels = .8,
     labelScale = FALSE,
     featVecCol = phyl_ps_prune[names_clust],
     nodeColor = "feature",
     # nodeColor = "colorVec",
     # colorVec = list(plot_cluster$nodecolor$nodecol1[names_clust],
     #              plot_cluster$nodecolor$nodecol2[names_clust]),
     sameClustCol = FALSE,
     borderCol = "gray40", highlightHubs = FALSE,
...
![](5.2.1_Networks_viz_Gen_files/figure-gfm/unnamed-chunk-11-2.png)<!-- -->
