...
```{r, echo=FALSE}
load('/Users/alicesommer/Desktop/Bureau/DOCTORATE/data_pipeline_microbiome/ps_to_net.RData')
...
```{r, echo=TRUE, fig.height=9, fig.width=15}
...
set.seed(1)
props_W_igraph <- graph_from_adjacency_matrix(props_W$input$adjaMat1, # I used assoMat1 before
                                              #mode = "undirected",
                                              weighted = TRUE)
...
#### Module 5 (clean air) split

```{r, fig.height=8, fig.width=15}
names_clust <- as.character(names(which(props_W$clustering$clust2 == 5)))
col_module <- unique(plot_cluster$q1$Arguments$color[names_clust],
                     plot_cluster$q2$Arguments$color[names_clust])
...
### Correlation structure of module 5
...
### "Bacteroides" and its neighbors

```{r}
# subset Seq286 and its neighbors
neigh_1 <- names(which(props_W$input$assoMat1[,248] > 0))
neigh_2 <- names(which(props_W$input$assoMat2[,248] > 0))

names_neigh <- unique(c(neigh_1, neigh_2))

labels_phyl <- substr(as.character(phyl_ps_prune[names_neigh]), 1, 3)
...
plot(props_W, 
     ...
     nodeFilter = "names",
     nodeShape = c("circle", "triangle"),
     featVecShape = shapeVec[names_neigh],
     nodeFilterPar = names_neigh,
     edgeTranspLow = 10, edgeTranspHigh = 10,
     groupNames = c("Polluted air", "Clean air"))
```
