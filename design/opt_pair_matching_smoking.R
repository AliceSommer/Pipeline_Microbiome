...
dat_excl_temp1 <- dat_transformed[(dat_transformed$u3tmabio_j01 %in% 0) & (dat_transformed$u3tmabio_son %in% 0), ]
...
data$W <- NA
data[data$u3tcigsmk == 1,]$W <- 0 # raucher
data[data$u3tcigsmk == 3,]$W <- 1 # nie-raucher
...
g <- grid.arrange(g_age, g_bmi, g_sex, g_alcohol, g_phys, g_educ, g_season,
...
g_after <- grid.arrange(arrangeGrob(g_age, g_bmi, g_sex, g_alcohol, g_phys, g_educ, g_season, 
                                    top="Before Matching", ncol = 2),
                        arrangeGrob(g_age_after, g_bmi_after, g_sex_after, g_alcohol_after, 
                                    g_phys_after, g_educ_after, g_season_after,
                                    top="After Matching", ncol = 2), ncol = 2)

# ggsave(file = 'plots_pipeline_microbiome/smoke_balance.jpeg',
#        g_after,
#        dpi=300,
#        width = 300,
#        height = 350,
...
# ggsave(file = 'plots_pipeline_microbiome/smoke_balance_other.jpeg',
#        g_after_other,
#        dpi=300,
#        width = 300,
#        height = 200,
...
# ggsave(file = 'plots_pipeline_microbiome/smoke_balance_medic.jpeg',
#        g_after_medic,
#        dpi=300,
#        width = 300,
#        height = 300,
...
# SAVE DATASET #
################
# save(matched_df, file = 'data_pipeline_microbiome/dat_matched_smoke_bis.RData')
...
W_paired_smoke <- W_unique[order(matched_df$ff4_prid),]

save(W_paired_smoke, file = "data_pipeline_microbiome/W_paired_smoke_bis.Rdata")
