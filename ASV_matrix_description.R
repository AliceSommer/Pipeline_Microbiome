...
prev_labels <- list(
  '1'="0 % - p = 4,370",'2'="5 % - p = 515",'3'="10 % - p = 311",
  '4'="15 % - p = 222",'5'="20 % - p = 163",'6'="25 % - p = 132",
  '7'="30 % - p = 111", '8'="35 % - p = 95", '9'="40 % - p = 73"
)
...
g_1 <- ggplot(dat_plot, aes(x = values)) +
  ...
  geom_text(data = dat_text_lab, mapping = aes(x = 5000, y = 50, label = min_count), 
            colour = "red") + 
...
g_2 <- ggplot(dat_plot_2, aes(x = values)) +
  ...
  facet_wrap(~filter, labeller=prev_labeller) +
  theme(axis.text.x = element_text(angle=90)) +
  geom_text(data = dat_text_lab, mapping = aes(x = 10, y = 50, label = min_ASV),
            colour = "red") +
